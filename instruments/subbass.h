...
// sub-bass instrument (single variant)
...
// The sound is tuned for a deep, powerful sub-bass with smooth sustain and minimal attack
...
private:
    ...
    AudioUtils::LowPassFilter lowPass;      // Emphasizes low-end for sub-bass depth
    AudioUtils::Distortion distortion;      // Adds subtle harmonic richness
...
public:
    SubBass(double gainValue = 0.85) // Moderate gain for deep presence in mix
        : protector(0.01, 0.8),       // 10ms fade-out, 80% max gain for smooth control
          lowPass(100.0),             // 100Hz cutoff for deep sub-bass focus
          distortion(1.2, 0.95, 1.1), // Subtle distortion: drive=1.2, threshold=0.95, soft=1.1
          envFollow(0.005, 0.1),      // Smooth envelope: 5ms attack, 100ms release
          gain(gainValue)             // Volume 0.0 through 1.0
    {}

    // Generate a sub-bass sound at time t, frequency freq, duration dur
    double generateWave(double t, double freq, double dur) override {
        // Constrain frequency to sub-bass range (20Hz to 100Hz for deep tones)
        freq = std::max(20.0, std::min(100.0, freq));

        // ADSR envelope: smooth attack, long sustain for sub-bass
        double attack = 0.01, decay = 0.2, sustain = 0.8, release = 0.5;
        double env;
        if (t < attack) {
            env = t / attack;
        ...
        // Pitch envelope for subtle tonal warmth
        double pitchEnv = std::exp(-t / 0.3) * 20.0;
        double pitchMod = freq + pitchEnv;

        // Waveforms: Tonal-based for sub-bass depth
        double tonal = 0.8 * std::sin(2.0 * M_PI * pitchMod * t); // Primary sine wave
        double subharmonic = 0.2 * std::sin(2.0 * M_PI * (pitchMod / 2.0) * t); // Subharmonic for extra depth

        // Combine waveforms
        double output = env * (tonal + subharmonic);

        // Dynamic filter cutoff based on envelope follower
        double envValue = envFollow.process(std::abs(output));
        lowPass.setCutoff(80.0 + 40.0 * envValue); // 80Hz-120Hz for dynamic low-end

        // Apply effects chain
        output = distortion.process(output); // Add subtle harmonic richness
...
// Register the sub-bass instrument
static InstrumentRegistrar<SubBass> regSubBass("subbass");
