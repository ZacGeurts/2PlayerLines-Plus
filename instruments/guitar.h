...
// The sound is tuned for a bright, resonant guitar tone with clear pluck and string sustain
...
private:
    ...
    AudioUtils::BandPassFilter bandPass;    // Emphasizes mid-range clarity
...
public:
    ...
    Guitar(double gainValue = 0.9, const std::string& instrumentName = "guitar")
        : protector(0.01, 0.92),         // 10ms fade-out, 92% max gain for clean output
          whiteNoise(-0.6L, 0.6L),       // White noise for velocity variation
          pinkNoise(0.09),               // Pink noise for string pluck texture
          lowPass(4000.0),               // 4kHz cutoff for bright, resonant tone
          highPass(80.0, 0.707),         // 80Hz cutoff, Q=0.707 to remove mud
          bandPass(1200.0, 0.8),         // 1.2kHz center, Q=0.8 for mid-range clarity
          distortion(1.6, 0.9, 1.8),     // Gritty distortion: drive=1.6, threshold=0.9, soft=1.8
          brownNoise(0.04),              // Brown noise for body resonance
          reverb(0.4, 0.7, 0.35, 0.1),   // 400ms delay, 70% decay, 35% mix for room ambiance
          chorus(0.3, 0.6, 0.2),         // Depth=0.3, rate=0.6Hz, mix=20% for string resonance
          tremolo(6.0, 0.12),            // Rate=6Hz, depth=12% for subtle vibrato
          envFollow(0.005, 0.1),         // 5ms attack, 100ms release for dynamic response
          gain(gainValue),
          name(instrumentName)          // Store name for variant handling
    {} // Empty constructor body

    // Generate a guitar sound at time t, frequency freq, duration dur
    double generateWave(double t, double freq, double dur) override {
        // Constrain frequency to guitar range (82Hz to 1.3kHz for E2 to E5)
        ...
        // ADSR envelope for clear plucked tone
        double attack = 0.005, decay = 0.15, sustain = 0.65, release = 0.25; // Guitar envelope
        if (name == "guitar_bright") {
            attack = 0.003; decay = 0.1; sustain = 0.7; // Brighter, sharper pluck
        } else if (name == "guitar_clean") {
            attack = 0.008; sustain = 0.6; release = 0.3; // Softer, cleaner tone
        }
        double env;
        if (t < attack) {
            env = t / attack;
        ...
        double noise = 0.1 * pinkNoise() * std::exp(-t / 0.02);             // String pluck
        ...
        double lowPassCutoff = 4000.0, bandPassCenter = 1200.0;
        if (name == "guitar_bright") {
            mixSine3 *= 1.4; mixNoise *= 1.3; lowPassCutoff = 5000.0; bandPassCenter = 1500.0;
            distortion.setDrive(1.8); // Brighter tone
        } else if (name == "guitar_clean") {
            mixSine1 *= 1.2; mixSine3 *= 0.7; lowPassCutoff = 3500.0; bandPassCenter = 1000.0;
            distortion.setDrive(1.4); // Cleaner tone
        }
        ...
        bandPass.setCenterFreq(bandPassCenter + 300.0 * envValue); // Dynamic mid-range emphasis

        // Apply effects chain
        output = highPass.process(output);     // Remove mud
        output = bandPass.process(output);     // Emphasize clarity
...
// Register guitar variations
static InstrumentRegistrar<Guitar> regGuitar("guitar");
static InstrumentRegistrar<Guitar> regGuitarBright("guitar_bright");
static InstrumentRegistrar<Guitar> regGuitarClean("guitar_clean");
 ...
 * for high-quality instrument synthesis, particularly for realistic bass guitar sounds.
 ...
 *      - Dice rolling and coin flipping for game-like randomness.
 *      - Thread-safe with clock-free seeding.
 *    - Call Example:
 *      RandomGenerator rng(-1.0, 1.0); // Range [-1, 1]
 *      double white = rng.generateWhiteNoise(); // White noise sample
 *      double pink = rng.generatePinkNoise();   // Pink noise sample
 *      int dice = rng.roll_d6();                // Roll a 6-sided die
 *
 * 2. HighPassFilter
 ...
 * - Enhances bass guitar synthesis by adding noise textures, dynamic effects, and
 *   professional polish.
 */