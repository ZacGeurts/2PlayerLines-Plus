...
// The sound is tuned for a bright, shimmering cymbal tone with sharp attack and metallic sustain
...
private:
    ...
    AudioUtils::PinkNoise pinkNoise;        // Pink noise for cymbal shimmer
    ...
    AudioUtils::Chorus chorus;              // Thickens sound for shimmer
...
public:
    ...
    Cymbal(double gainValue = 0.9, const std::string& instrumentName = "cymbal")
        : protector(0.01, 0.92),         // 10ms fade-out, 92% max gain for clean output
          whiteNoise(-0.8L, 0.8L),       // White noise for velocity variation
          pinkNoise(0.25),               // Pink noise for cymbal shimmer
          lowPass(10000.0),              // 10kHz cutoff for bright, shimmering tone
          highPass(500.0, 0.707),        // 500Hz cutoff, Q=0.707 to remove mud
          bandPass(4000.0, 0.8),         // 4kHz center, Q=0.8 for metallic harmonics
          ...
          reverb(0.4, 0.7, 0.4, 0.1),    // 400ms delay, 70% decay, 40% mix for room ambiance
          chorus(0.35, 0.7, 0.25),       // Depth=0.35, rate=0.7Hz, mix=25% for shimmer
    ...
    // Generate a cymbal sound at time t, frequency freq, duration dur
    double generateWave(double t, double freq, double dur) override {
        // Constrain frequency to cymbal range (1kHz to 8kHz for shimmer range)
        freq = std::max(1000.0, std::min(8000.0, freq));
        ...
        // ADSR envelope for sharp attack with sustained shimmer
        double attack = 0.003, decay = 0.1, sustain = 0.5, release = 0.3; // Cymbal envelope
        if (name == "cymbal_bright") {
            attack = 0.002; decay = 0.08; sustain = 0.55; // Brighter, sharper strike
        } else if (name == "cymbal_muted") {
            attack = 0.005; sustain = 0.4; release = 0.2; // Softer, muted strike
        }
        double env;
        if (t < attack) {
            env = t / attack;
        ...
        double noise = 0.5 * pinkNoise() * std::exp(-t / 0.02);             // Shimmer texture
        double brown = 0.02 * brownNoise() * std::exp(-t / 0.1);           // Subtle resonance

        // Variant-specific adjustments
        double mixSine1 = 0.2, mixSine2 = 0.15, mixSine3 = 0.1, mixNoise = 0.5, mixBrown = 0.02;
        double lowPassCutoff = 10000.0, bandPassCenter = 4000.0;
        if (name == "cymbal_bright") {
            mixSine3 *= 1.5; mixNoise *= 1.4; lowPassCutoff = 12000.0; bandPassCenter = 5000.0;
            distortion.setDrive(2.0); // Brighter tone
        } else if (name == "cymbal_muted") {
            mixSine1 *= 1.2; mixNoise *= 0.7; lowPassCutoff = 8000.0; bandPassCenter = 3000.0;
        ...
        lowPass.setCutoff(lowPassCutoff - 1500.0 * envValue); // Dynamic cutoff for expressiveness
        bandPass.setCenterFreq(bandPassCenter + 600.0 * envValue); // Dynamic harmonic emphasis
        ...
        output = chorus.process(output);       // Shimmer
...
// Register cymbal variations
static InstrumentRegistrar<Cymbal> regCymbal("cymbal");
static InstrumentRegistrar<Cymbal> regCymbalBright("cymbal_bright");
static InstrumentRegistrar<Cymbal> regCymbalMuted("cymbal_muted");
