...
// The sound is tuned for a rich, expressive violin tone with warm sustain and subtle bow texture
...
public:
    ...
    Violin(double gainValue = 0.85, const std::string& instrumentName = "violin")
        : protector(0.015, 0.9),        // 15ms fade-out, 90% max gain for smooth output
          ...
          pinkNoise(0.07),              // Pink noise for bow texture
          lowPass(2000.0),              // 2kHz cutoff for warm, rounded tone
          highPass(50.0, 0.707),        // 50Hz cutoff, Q=0.707 to remove rumble
          bandPass(800.0, 0.9),         // 800Hz center, Q=0.9 for mid-range body
          distortion(1.4, 0.95, 2.0),    // Subtle distortion: drive=1.4, threshold=0.95, soft=2.0
          brownNoise(0.04),             // Brown noise for low-end resonance
          reverb(0.5, 0.75, 0.5, 0.1),   // 500ms delay, 75% decay, 50% mix for hall ambiance
    ...
    // Generate a violin sound at time t, frequency freq, duration dur
    double generateWave(double t, double freq, double dur) override {
        // Constrain frequency to violin range (196Hz to 3kHz for G3 to E6)
        freq = std::max(196.0, std::min(3000.0, freq));

        // Dynamic velocity with subtle variation
        double velocity = 0.9 + whiteNoise.generate() * 0.3; // Subtle variation for bow dynamics
        velocity = std::max(0.7, std::min(1.0, velocity));

        // ADSR envelope for smooth bowed attack
        double attack = 0.03, decay = 0.2, sustain = 0.85, release = 0.3; // Smooth bowed envelope
        if (name == "violin_pizzicato") {
            attack = 0.005; decay = 0.1; sustain = 0.5; release = 0.15; // Sharp pizzicato envelope
        } else if (name == "violin_solo") {
        ...
        // Pitch envelope for vibrato
        double pitchEnv = (name == "violin_pizzicato") ? 0.0 : std::sin(2.0 * M_PI * 5.0 * t) * 0.8; // 5Hz vibrato
        ...
        if (name == "violin_solo") {
            mixSine2 *= 1.2; mixSine3 *= 1.3; lowPassCutoff = 2500.0; 
            distortion.setDrive(1.6); // Brighter, expressive tone
        } else if (name == "violin_pizzicato") {
            mixNoise *= 1.5; mixBrown *= 0.8; lowPassCutoff = 1800.0; 
...
// Register violin variations
static InstrumentRegistrar<Violin> regViolin("violin");
static InstrumentRegistrar<Violin> regViolinSolo("violin_solo");
static InstrumentRegistrar<Violin> regViolinPizzicato("violin_pizzicato");
