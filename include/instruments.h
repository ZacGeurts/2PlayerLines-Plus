...
class HighPassFilter {
    float cutoffFreq, sampleRate, x1, y1;
public:
    HighPassFilter(float cutoff, float q, float sr) : cutoffFreq(cutoff), sampleRate(sr), x1(0.0f), y1(0.0f) {}
    float process(float input) {
        float omega = 2.0f * M_PI * cutoffFreq / sampleRate;
        float alpha = std::sin(omega) / (2.0f * 0.707f); // Q = 0.707 for Butterworth response
...
// Wave generation function declarations
